...
char *get_path(char *path) {
  // Check if path is relative and prefix './' if needed.
  if (path[0] != '/' || path[0] != '\\') {
    char *prefix = "./";
...
int main(int argc, char *argv[]) {
  ...
  struct sudoku *sudoku = calloc(1, sizeof(struct sudoku));
  if (sudoku == NULL) {
    fprintf(stderr, "error: calloc returned NULL.\n");
  }

  sudoku_init(sudoku, verbose);
  if (sudoku_load(input, sudoku) != 0) {
    fprintf(stderr, "error: unable to load sudoku.\n");
    return 1;
  }

  if (sudoku_solve(sudoku) != 0) {
    fprintf(stderr, "error: unable to solve sudoku.\n");
    return 1;
  }

  if (sudoku_save(output, sudoku) != 0) {
